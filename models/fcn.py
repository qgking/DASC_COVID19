...
class FCN8s(nn.Module):
    ...
    def __init__(self, pretrained_net=None, n_channels=5, n_class=3):
        super().__init__()
        self.n_class = n_class
        self.pretrained_net = pretrained_net
        self.relu = nn.ReLU(inplace=True)
    ...
    def get_backbone_layers(self):
        small_lr_layers = []
        small_lr_layers.append(self.pretrained_net)
    ...
    def forward(self, x, cam):
        ...
        return [score], (x5, x5, x5), None  # size=(N, n_class, x.H/1, x.W/1)
...
def fcn8s(backbone='resnet50', n_channels=3, n_classes=1):
    vgg_model = VGGNet(requires_grad=True, pretrained=False)
    fcn_model = FCN8s(pretrained_net=vgg_model, n_channels=n_channels, n_class=n_classes)
    cov1_weight = fcn_model.pretrained_net.features[0].weight.data
    fcn_model.pretrained_net.features[0] = nn.Conv2d(n_channels, 64, kernel_size=3, stride=1, padding=1)
    nv = torch.unsqueeze(torch.mean(cov1_weight, dim=1), dim=1)
    fcn_model.pretrained_net.features[0].weight.data.copy_(nv)
    return fcn_model
