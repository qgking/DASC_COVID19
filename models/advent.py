...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, n_channels, num_classes, multi_level):
        self.inplanes = 64
        ...
        self.layer5 = self._make_pred_layer(ClassifierModule, 256, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
        self.layer6 = self._make_pred_layer(ClassifierModule, 512, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
    ...
    def forward(self, x, cam):
        ...
        x1 = self.layer5(backbone_out.layer3)  # produce segmap 1
        x2 = self.layer6(backbone_out.layer4)  # produce segmap 2
...
def ADVENT(backbone='resnet', n_channels=1, n_classes=19, multi_level=True):
    model = ResNetMulti(Bottleneck, [3, 4, 23, 3], n_channels, n_classes, multi_level)
    return model
