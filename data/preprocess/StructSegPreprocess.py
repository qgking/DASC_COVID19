...
def process_lung_cancer_nii_file(new_img_dir='Lung_GTV_Processed', truncate=[MIN_IMG_BOUND, MAX_IMG_BOUND],
                                 root_dir='../../medical_data/COVID193DCT/StructSeg', resample=False,
                                 new_voxel_dim=[1, 1, 1],
                                 min_max=False):
    raw_file_dir = join(root_dir, 'Lung_GTV')
    ...
    for ff in files:
        print(join(raw_file_dir, ff, 'data.nii.gz'))
        scan = nib.load(join(raw_file_dir, ff, 'data.nii.gz'))
        img = scan.get_data()
        print(img.shape)
        ...
        msk = nib.load(join(raw_file_dir, ff, 'label.nii.gz')).get_data()
        if resample:
        ...
        # sio.savemat(join(new_file_dir, 'train_' + ff + '.mat'), mdict={'STRUCT_CANCER': final})
        np.save(join(new_file_dir, 'train_' + ff + '.npy'), final)
        generate_lungtxt(msk, join(new_file_dir, 'train_' + ff + '_abnomal'))


def process_lung_nii_file(new_img_dir='Thoracic_Processed', truncate=[MIN_IMG_BOUND, MAX_IMG_BOUND],
                          root_dir='../../medical_data/COVID193DCT/StructSeg', resample=False, new_voxel_dim=[1, 1, 1],
                          min_max=False):
    raw_file_dir = join(root_dir, 'Thoracic_OAR')
    new_img_dir = new_img_dir + '_%d' % (truncate[1])
    new_file_dir = join(root_dir, new_img_dir)
    if resample:
        new_file_dir = new_file_dir + '_Resample'
    ...
    for ff in files:
        ...
        print(np.unique(msk))
        final = np.stack([img, msk])
        # # sio.savemat(join(new_file_dir, 'train_' + ff + '.mat'), mdict={'STRUCT_LUNG': final})
        np.save(join(new_file_dir, 'train_' + ff + '.npy'), final)
        generate_lungtxt(msk, join(new_file_dir, 'train_' + ff + '_lung'))
...
if __name__ == '__main__':
    ...
    process_lung_nii_file(new_img_dir='Thoracic_Processed', truncate=[MIN_IMG_BOUND, MAX_IMG_BOUND],
                          root_dir=join(root, 'StructSeg'), resample=False,
                          new_voxel_dim=[1, 1, 1],
                          min_max=False)
    # root = '../../../medical_data/COVID193DCT/'
    # process_lung_cancer_nii_file(new_img_dir='Lung_GTV_Processed', truncate=[MIN_IMG_BOUND, MAX_IMG_BOUND],
    #                              root_dir=join(root, 'StructSeg'), resample=False,
    #                              new_voxel_dim=[1, 1, 1],
    #                              min_max=False)
