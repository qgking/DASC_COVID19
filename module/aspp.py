...
class ASPPModule(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size, padding, dilation, batchnorm):
        ...
        self.atrous_conv = nn.Conv3d(inplanes, planes, kernel_size=kernel_size, stride=1, padding=padding,
...
class ASPP(nn.Module):
    ...
    def __init__(self, backbone, output_stride, batchnorm):
        ...
        if backbone == 'vgg16bn':
            inplanes = 512
        elif backbone == 'resnet101' or backbone == 'resnet152' or backbone == 'resnet50':
            inplanes = 2048
        ...
        self.global_average_pool = nn.Sequential(nn.AdaptiveAvgPool3d((1, 1, 1)),
...
class SegASPPDecoder(nn.Module):
    def __init__(self, num_classes, backbone):
        ...
        if backbone == 'resnet101' or backbone == 'resnet152' or backbone == 'resnet50':
    ...
    def forward(self, x, low_level_feat):
        ...
        x = F.interpolate(x, low_level_feat.size()[2:], mode='trilinear', align_corners=True)
...
if __name__ == '__main__':
    aspp = ASPP(backbone='resnet50', output_stride=8, batchnorm=nn.BatchNorm3d)
    input = torch.rand(1, 2048, 32, 32, 32)
    output = aspp(input)
