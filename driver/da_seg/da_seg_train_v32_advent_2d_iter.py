...
def train(seg_help, model_cam, train_loader, unsu_loader, latent_discriminator,
    ...
    for i_iter in range(total_iter):
        ...
        loss_su_seg = 0.1 * seg_help.criterions['loss'](pred_source1, su_label) \
                      + 1.0 * seg_help.criterions['loss'](pred_source2, su_label)
        ...
        d_out_seg = seg_discriminator(seg_help.prob_2_entropy(F.softmax(pred_target2, dim=1)))
        d_out_feature = latent_discriminator(seg_help.prob_2_entropy(F.softmax(pred_target1, dim=1)))
        ...
        loss_adv_t = loss_adv * 0.001 + loss_aux * 0.0002
        loss_adv_t.backward()
        ...
        D_out_s = seg_discriminator(seg_help.prob_2_entropy(F.softmax(pred_source2, dim=1)))
        D_out_sf = latent_discriminator(seg_help.prob_2_entropy(F.softmax(pred_source1, dim=1)))
        y_truth_tensor = get_y_truth_tensor(D_out_s, su_label_tag)
        y_truth_tensor_feature = get_y_truth_tensor(D_out_sf, su_label_tag)

        loss_D_s = seg_help.criterions['bceloss'](D_out_s, y_truth_tensor) / 2
        loss_D_sf = seg_help.criterions['bceloss'](D_out_sf, y_truth_tensor_feature) / 2
        loss_D_s.backward()
        loss_D_sf.backward()
        ...
        D_out_t = seg_discriminator(seg_help.prob_2_entropy(F.softmax(pred_target2, dim=1)))
        D_out_tf = latent_discriminator(seg_help.prob_2_entropy(F.softmax(pred_target1, dim=1)))
        y_truth_tensor = get_y_truth_tensor(D_out_t, un_label_tag)
        y_truth_tensor_feature = get_y_truth_tensor(D_out_tf, un_label_tag)

        loss_D_t = seg_help.criterions['bceloss'](D_out_t, y_truth_tensor) / 2
        loss_D_tf = seg_help.criterions['bceloss'](D_out_tf, y_truth_tensor_feature) / 2
        loss_D_t.backward()
        loss_D_tf.backward()
